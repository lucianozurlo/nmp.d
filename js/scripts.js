  ...
  function CustomFunction () {
    ...
    function loadSequenceImages (prefix, start, end, pageName) {
      ...
      for (let i = start; i <= end; i++) {
        ...
        paths.push (`/projects/_seq/${prefix}${paddedIndex}.png`);
      }
      return paths;
    }
    ...
    function loadImagesForPage (pageName) {
      const jsonPath = `/projects/_json/${pageName}.json`; // Ruta absoluta desde la raíz
      ...
      fetch (jsonPath)
        ...
        .then (data => {
          ...
          const vidPaths = data.vid
            ? data.vid.map (video => `/projects/_vid/${video}`)
            : [];
